...
export const fetchWatchlist = (movie) => {
  return {
...
export const handleFetchWatchlist = () => {
  return async (dispatch) => {
    const response = await fetch(
      "https://movie-78f07-default-rtdb.firebaseio.com/watchlist.json"
    );
...
export const handleDeleteFromWatchlist = (id) => {
  return async (dispatch) => {
    const response = await fetch(
      `https://movie-78f07-default-rtdb.firebaseio.com/watchlist.json/${id}.json`,
      {
        method: "DELETE",
      }
    );
